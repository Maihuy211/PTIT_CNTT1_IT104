...
class Classroom {
   ...
   removeStudent(allStudents: Student[], studentId: number): void {
    const remove = this.students.find(s => s.getId() === studentId);
    if (remove) {
        allStudents.push(remove);
...
class1.removeStudent(allStudents, 2);
class2.editStudent(4, "quach bach");
